...
This creates an S3 bucket with server-side encryption enabled using AWS-managed encryption keys (SSE-S3).
"""
...
class S3BucketEncryptedStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        bucket = s3.Bucket(
            ...
            auto_delete_objects=False,  # Do not delete objects on stack deletion
        )
        ...
        CfnOutput(
            self,
            "SecurityInfo",
            value="This bucket has server-side encryption enabled using AWS-managed keys (SSE-S3).",
            description="Security information about encrypted bucket",
        )
